...
import { freeLearningCatalog, type FreeLearningKind } from "@/data/freeLearningCatalog";
...
export default function Certifications() {
  ...
  const [kindFilter, setKindFilter] = useState<FreeLearningKind | "Todos">("Todos");
  const progressQuery = trpc.externalLearning.progress.useQuery(undefined, { enabled: Boolean(user) });
  const toggleProgressMutation = trpc.externalLearning.toggleProgress.useMutation({
    onSuccess: () => progressQuery.refetch(),
  });
  const providers = ["Todos", ...Array.from(new Set(freeLearningCatalog.map(resource => resource.provider)))];
  const levels = ["Todos", ...Array.from(new Set(freeLearningCatalog.map(resource => resource.level)))];
  const kinds: Array<FreeLearningKind | "Todos"> = ["Todos", "Curso gratuito", "Microcurso com certificado", "Badge / credencial"];
  const filteredFreeResources = useMemo(() => freeLearningCatalog.filter(resource => (providerFilter === "Todos" || resource.provider === providerFilter) && (levelFilter === "Todos" || resource.level === levelFilter) && (kindFilter === "Todos" || resource.kind === kindFilter)), [providerFilter, levelFilter, kindFilter]);
  const completedResourceIds = useMemo(() => new Set((progressQuery.data ?? []).filter(item => item.completed === 1).map(item => item.resourceId)), [progressQuery.data]);
    ...
    <div className="w-full">
      ...
      <section className="py-16 border-b border-border bg-primary/5">
        <div className="container space-y-8">
          ...
          <div className="grid gap-4 rounded-xl border border-border bg-card/60 p-4 md:grid-cols-4" aria-label="Filtros de recursos gratuitos"><label className="text-xs font-semibold text-muted-foreground">Provedor<select value={providerFilter} onChange={event => setProviderFilter(event.target.value)} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground"><option value="Todos">Todos</option>{providers.filter(provider => provider !== "Todos").map(provider => <option key={provider} value={provider}>{provider}</option>)}</select></label><label className="text-xs font-semibold text-muted-foreground">Nível<select value={levelFilter} onChange={event => setLevelFilter(event.target.value)} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground"><option value="Todos">Todos</option>{levels.filter(level => level !== "Todos").map(level => <option key={level} value={level}>{level}</option>)}</select></label><label className="text-xs font-semibold text-muted-foreground">Modalidade<select value={kindFilter} onChange={event => setKindFilter(event.target.value as FreeLearningKind | "Todos")} className="mt-2 w-full rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground">{kinds.map(kind => <option key={kind} value={kind}>{kind}</option>)}</select></label><div className="flex items-end"><div className="w-full rounded-md border border-primary/20 bg-primary/5 px-3 py-2 text-sm"><strong className="block text-primary">{completedCount}/{freeLearningCatalog.length}</strong><span className="text-xs text-muted-foreground">concluídos {user ? "na sua conta" : "— entre para salvar"}</span></div></div></div>
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">{filteredFreeResources.map((resource) => { const completed = completedResourceIds.has(resource.id); return <article key={resource.id} className="flex flex-col rounded-xl border border-border bg-card p-5 transition hover:-translate-y-0.5 hover:border-primary/50"><div className={`mb-4 h-1.5 rounded-full bg-gradient-to-r ${resource.color}`} /><p className="text-xs font-semibold uppercase tracking-wider text-primary">{resource.kind}</p><h3 className="mt-2 text-lg font-bold">{resource.title}</h3><p className="mt-1 text-xs text-muted-foreground">{resource.provider} · {resource.level}</p><p className="mt-3 flex-1 text-sm leading-6 text-muted-foreground">{resource.description}</p><div className="mt-4 flex flex-wrap gap-2">{resource.topics.map(topic => <span key={`${resource.id}-${topic}`} className="rounded bg-muted px-2 py-1 text-[11px] text-muted-foreground">{topic}</span>)}</div><p className="mt-4 text-xs leading-5 text-muted-foreground"><strong className="text-foreground">Acesso:</strong> {resource.accessNote}</p><div className="mt-5 grid gap-2"><a href={resource.url} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-md border border-primary/35 px-3 py-2 text-sm font-semibold text-primary transition hover:bg-primary/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Abrir fonte oficial <ArrowRight className="size-4" /></a><button type="button" disabled={!user || toggleProgressMutation.isPending} onClick={() => toggleProgressMutation.mutate({ resourceId: resource.id, resourceKind: resource.kind, completed: !completed })} className={`inline-flex items-center justify-center gap-2 rounded-md px-3 py-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring ${completed ? "bg-emerald-500/10 text-emerald-300" : "bg-muted text-muted-foreground hover:bg-muted/80"}`} aria-label={user ? `${completed ? "Desmarcar" : "Marcar"} ${resource.title} como concluído` : "Entre para acompanhar o progresso"}>{completed ? <CheckCircle2 className="size-4" /> : <Award className="size-4" />}{completed ? "Concluído" : user ? "Marcar como concluído" : "Entrar para acompanhar"}</button></div></article>; })}</div>
          {filteredFreeResources.length === 0 && <p className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Nenhum recurso corresponde aos filtros selecionados.</p>}
        </div>
      </section>
