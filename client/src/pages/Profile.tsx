...
import { Award, ArrowRight, CalendarDays, CheckCircle2, Clock3, Linkedin, Medal, Pencil, Save, ShieldCheck, Sparkles, Trophy, Upload, UserRound } from "lucide-react";
...
import { getInitials, readProfilePreferences, writeProfilePreferences } from "@/data/profile";
...
export default function Profile() {
  ...
  const profileShareBase = typeof window !== "undefined" ? window.location.href : "https://ialearnhub-ndm4gtgm.manus.space/profile";
  ...
  const handleSaveProfile = () => {
    const nextPreferences = { name: profileName.trim(), bio: profileBio.trim(), avatarDataUrl };
  ...
  return <div className="container py-12 space-y-10">
    <section className="relative overflow-hidden rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/15 via-card to-secondary/10 p-8 md:p-10"><div className="absolute -right-12 -top-16 size-48 rounded-full bg-primary/20 blur-3xl" aria-hidden="true" /><div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between"><div className="flex items-center gap-5"><div className="grid size-20 shrink-0 place-items-center overflow-hidden rounded-3xl bg-gradient-to-br from-primary to-secondary text-2xl font-black text-white shadow-lg shadow-primary/25" aria-label={`Avatar de ${displayName}`}>{avatarDataUrl ? <img src={avatarDataUrl} alt={`Avatar de ${displayName}`} className="size-full object-cover" /> : initials}</div><div><p className="text-sm font-semibold uppercase tracking-wider text-primary">Meu perfil de aprendizagem</p><h1 className="mt-1 text-3xl font-bold tracking-tight md:text-4xl">{displayName}</h1><p className="mt-2 max-w-xl text-muted-foreground">{profileBio || "Acompanhe suas conquistas, simulados e evolução em IA."}</p></div></div><div className="flex flex-wrap gap-3"><Button type="button" variant="outline" onClick={() => setIsEditorOpen(true)} className="gap-2"><Pencil className="size-4" /> Editar perfil</Button><Button asChild className="gap-2"><a href="/interactive-certifications">Continuar certificações <ArrowRight className="size-4" /></a></Button></div></div></section>
    {savedNotice && <div role="status" className="rounded-2xl border border-emerald-400/30 bg-emerald-400/10 px-4 py-3 text-sm font-medium text-emerald-300">Perfil atualizado com sucesso neste navegador.</div>}

    <section aria-labelledby="study-stats-heading" className="space-y-4"><div><p className="text-xs font-semibold uppercase tracking-wider text-primary">Visão de progresso</p><h2 id="study-stats-heading" className="text-2xl font-bold">Suas estatísticas de estudo</h2></div><div className="grid gap-4 md:grid-cols-[1fr_1.4fr]"><div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-center gap-2 text-muted-foreground"><Clock3 className="size-5 text-primary" /><span className="text-sm font-medium">Tempo total estimado de estudo</span></div><p className="mt-4 text-4xl font-bold">{formatStudyTime(studyHours)}</p><p className="mt-2 text-sm text-muted-foreground">Calculado a partir das aulas marcadas como concluídas na trilha.</p></div><div className="rounded-2xl border border-border bg-card p-6"><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2 text-muted-foreground"><Trophy className="size-5 text-amber-400" /><span className="text-sm font-medium">{nextBadge ? "Próxima medalha" : "Percurso completo"}</span></div><h3 className="mt-3 text-xl font-bold">{nextBadge?.label || "Todas as medalhas conquistadas"}</h3><p className="mt-1 text-sm text-muted-foreground">{nextBadge?.description || "Você concluiu todas as conquistas desta etapa."}</p></div><span className="rounded-full bg-primary/10 px-3 py-1 text-sm font-bold text-primary">{nextBadgeProgress}%</span></div><div className="mt-5 h-3 overflow-hidden rounded-full bg-muted" role="progressbar" aria-label={`Progresso até ${nextBadge?.label || "todas as medalhas"}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={nextBadgeProgress}><div className="h-full rounded-full bg-gradient-to-r from-primary via-secondary to-amber-400 transition-[width] duration-500" style={{ width: `${nextBadgeProgress}%` }} /></div><p className="mt-2 text-xs text-muted-foreground">{nextBadge ? `${badges.filter(badge => badge.unlocked).length} de ${badges.length} medalhas desbloqueadas` : "Parabéns pela consistência!"}</p></div></div></section>
